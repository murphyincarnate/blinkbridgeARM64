...
class CameraManager:
    ...
    async def _login(self) -> None:
        ...
        # Headless path if creds already saved
        if path_cred.exists():
            log.debug("logging into Blink with saved creds")
            self.blink.auth = Auth(await json_load(path_cred), no_prompt=True)
            await self.blink.start()
            return

        # Bootstrap path: username/password, then prompt for 2FA
        base_creds = {
            'username': CONFIG['blink']['login']['username'],
            'password': CONFIG['blink']['login']['password'],
        }
        self.blink.auth = Auth(base_creds, no_prompt=True)
        ...
        try:
            ...
            try:
                ...
                pass
            self.blink.auth.data['otp'] = code
            await self.blink.setup_post_verify()
            await self.blink.start()

        log.debug("saving Blink creds")
